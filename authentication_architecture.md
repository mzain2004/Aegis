# Aegis Authentication Architecture

Aegis implements a robust, secure, and modular authentication system designed to trace every administrative action and infrastructure mutation back to a verified human operator.
...
Aegis uses **Local API Keys** for primary operator authentication. 
...
- Aegis uses `argon2-cffi` to compute salted, secure hashes of the API keys.
...
To facilitate local development and automated testing, Aegis supports two configuration toggles in `Settings`:
...
To integrate SSO, OIDC, OAuth2, or JWT tokens in the future, developers only need to swap the `get_current_operator` implementation in [app/dependencies.py](file:///D:/Aegis/app/dependencies.py) without touching any business logic in routers or execution services.
