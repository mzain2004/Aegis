# Veto Ops Security Model

Veto Ops enforces a defense-in-depth security model to protect infrastructure mutation requests and human approval paths from malicious tampering, side-channel attacks, and credential leakages.
...
- **Fallback Verification scan**: If a client provides an API key without a username prefix, Veto Ops executes key verification scans against registered operators. This fallback scanning guarantees that even key guessing attempts on un-prefixed keys cannot be exploited via response timing differences.

## 3. Replay Protection

- **State Machine Enforcement**: Veto Ops enforces a single-direction state machine for approval request lifecycles:
  ```
...
- **Sensitive Field Suppression**: Veto Ops explicitly sanitizes logs to prevent accidental leaks. API keys, hashes, and HMAC signatures are stripped or redacted before log serialization.
